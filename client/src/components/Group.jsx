...
export default function Group({ data: gdData }) {
  ...
  const percentage = Math.round((gdData.total / 50) * 100);
    ...
    <div className="bg-white rounded-2xl shadow-sm p-4 lg:p-6">
      ...
      <div className="bg-gray-50 rounded-xl p-4 mb-6">
        <p className="text-sm">
          ...
          {gdData.topic || "Group Discussion Topic"}
        </p>
        <p className="text-sm text-gray-500 mt-1">
          <span className="font-semibold text-gray-700">Duration:</span> 20 mins
        </p>
      </div>
      ...
      <div className="flex flex-col lg:grid lg:grid-cols-2 lg:gap-8 lg:items-center">
        ...
        <div className="mt-6 lg:mt-0 space-y-4">
          <ScoreRow label="Subject Knowledge" value={`${gdData.subject_knowledge}/10`} />
          <ScoreRow label="Communication Skills" value={`${gdData.communication_skills}/10`} />
          <ScoreRow label="Body Language" value={`${gdData.body_language}/10`} />
          <ScoreRow label="Listening Skills" value={`${gdData.listening_skills}/10`} />
          <ScoreRow label="Active Participation" value={`${gdData.active_participation}/10`} />
